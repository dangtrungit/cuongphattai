<?php function amp_breadcrumb_output(){
    ...
    if ( !ampforwp_polylang_front_page() && !is_front_page() ) {
        ...
        // Home page 
        echo '<li class="item-home"><a class="bread-link bread-home" href="' . esc_url(ampforwp_url_controller( get_home_url('', '/'), $home_non_amp )) . '" title="' . $home_title . '">Trang chủ</a></li>';
        ...
        if ( is_archive() && !is_tax() && !is_category() && !is_tag() && !is_author() ) {
            ...
            // Custom
            global $post;
            echo '<li class="item-current item-post"><span class="bread-current bread-post">'. $post->post_title .'</span></li>';
