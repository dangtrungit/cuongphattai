...
// Post date
function amp_post_date(){
	global $redux_builder_amp, $post;
	?>
	<div class="poka-date"><i class="fa fa-clock-o" aria-hidden="true"></i>
	 	<span class="posted-on"> <a href="<?php echo get_permalink($post->ID); ?>" rel="bookmark"><time class="entry-date published" datetime="<?php echo $post->post_date; ?>"><?php echo date("d/m/Y", strtotime($post->post_date)); ?></time></a></span>
	</div>
	<?php
}

// Readmore
function amp_read_more(){
	global $redux_builder_amp, $post;
	?>
	<div class="rm">
	 	<a href="<?php echo amp_loop_permalink(true); ?>" class="readmore">Xem thêm <i class="fa fa-chevron-circle-right" aria-hidden="true"></i></a>
	</div>
	<?php
}
...
function get_paginate_links()
{
	...
	return '<div class="pagination nav-links">'.paginate_links($args).'</div>';
