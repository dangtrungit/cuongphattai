...
//     }else{
//      if(isset($vars['name'])){
//       $exists = $wpdb->get_var( $wpdb->prepare( "SELECT t.term_id FROM $wpdb->terms t LEFT JOIN $wpdb->term_taxonomy tt ON tt.term_id = t.term_id WHERE tt.taxonomy = 'category' AND t.slug = %s" ,array( $slug )));
      
//       if( $exists ){
//        $old_vars = $vars;
       
//        $vars = array('category_name' => $slug );
//        if(!empty($old_vars['paged']) || !empty($old_vars['page'])){
//         $vars['paged'] = !empty($old_vars['paged']) ? $old_vars['paged'] : $old_vars['page'];
//        }
//        if(!empty($old_vars['orderby'])){
...
add_filter('request', function( $vars ) {
  global $wpdb;
  if(isset($vars['name']) && $vars['name'] == 'amp'){
   //Xoa bo chuyen muc san pham
...
if ( ! function_exists('ampforwp_custom_rewrite_rules_for_product_category') ) {
	function ampforwp_custom_rewrite_rules_for_product_category(){
		if ( class_exists('WooCommerce') ) {
			...
			// For product category page - Pagination
			add_rewrite_rule( 
			...
			add_rewrite_rule(
			      ...
			      'index.php?amp=1&product_cat=$matches[1]',
			      'top'
			    );

			// For product category page - Pagination
			add_rewrite_rule( 
				 'cua-hang\/amp\/page\/?([0-9]{1,})/?$',
				 'index.php?post_type=product&paged=$matches[1]&amp=1',
				 'top' 
				);	
			...
			add_rewrite_rule(
			      'cua-hang\/amp/?$',
			      'index.php?amp=1&post_type=product',
			      'top'
			    );
			...
			//       'index.php?amp=1&product_tag=$matches[1]',
			//       'top'
			//     );
		 }
	}
