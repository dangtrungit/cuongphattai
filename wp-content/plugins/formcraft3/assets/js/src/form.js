...
class FormCraft {
  ...
  constructor(form) {
    ...
    form.on('input', '.oneLineText-cover input[type="text"], .password-cover input[type="password"], .datepicker-cover input[type="text"], .email-cover input[type="text"], .email-cover input[type="email"], .textarea-cover textarea', function() {
      self.setValue = []
    ...
    setTimeout(function() {
      ...
      self.form.parents('.form-live').find('.pre-populate-data').each(function() {
        let dataTemp = jQuery(this).text().replace(/“/g, '"').replace(/”/g, '"')
...
window.FormCraftSubmitForm = function(element, type, callback) {
  ...
  if (form.find('.form-element-type-submit.state-hidden').length === 1) {
    return false
  }
  ...
  .done(function(response) {
    ...
    if (response.failed) {
      ...
      form.append('<div class="final-success"><i class="icon-ok-circle"></i><span></span></div>')
...
jQuery(document).ready(function() {
  ...
  jQuery('.fc-form-modal').each(function() {
    ...
    if (jQuery(this).find('.pagination-trigger').length > 1) {
      jQuery(this).find('.fc_close').css('margin-top', '68px')
    }
  })
  jQuery('.fc-sticky').each(function() {
    if (jQuery(this).attr('data-auto') && !isNaN(parseFloat(jQuery(this).attr('data-auto')))) {
